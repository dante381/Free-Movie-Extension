    ...
    .then(async (response) => {
      ...
      if (data.status === "ok") {
        ...
        for (var movie in data.data.movies) {
          ...
          quality:`${data.data.movies[movie].torrents[torrent].quality} ${data.data.movies[movie].torrents[torrent].type} ${data.data.movies[movie].torrents[torrent].video_codec}`,
